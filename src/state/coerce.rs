...
/// JSON number → `u8`, defaulting to 0 on type mismatch.
pub fn json_u8(v: &Value) -> u8 {
    v.as_u64().unwrap_or(0).min(255) as u8
}

/// JSON bool → `bool`, defaulting to `false` on type mismatch.
